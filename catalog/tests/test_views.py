...
class AuthorFormViewTest(TestCase):
    ...
    def setUpTestData(cls):
        test_user1 = User.objects.create_user(username='testuser1', password='12345')
        test_user1.user_permissions.add(Permission.objects.get(codename="add_author"))
        test_user1.user_permissions.add(Permission.objects.get(codename="change_author"))
        test_user1.user_permissions.add(Permission.objects.get(codename="delete_author"))
        test_user1.save()
    ...
    def test_redirect_if_login_but_not_correct_permission(self):
        self.client.login(username='testuser2', password='12345')
        resp = self.client.get(reverse('author_create'))
        self.assertEqual(403, resp.status_code)

    def test_logged_user_with_permission_can_add_author(self):
        self.client.login(username='testuser1', password='12345')
        resp = self.client.get(reverse('author_create'))
        self.assertEqual(200, resp.status_code)
        self.assertTemplateUsed(template_name='catalog/author_form.html')

    def test_logged_user_with_permission_can_change_author(self):
        self.client.login(username='testuser1', password='12345')
        resp = self.client.get(reverse('author_update', kwargs={'pk': 1}))
        self.assertEqual(200, resp.status_code)
        self.assertTemplateUsed(template_name='catalog/author_form.html')

    def test_logged_user_with_permission_can_delete_author(self):
        self.client.login(username='testuser1', password='12345')
        resp = self.client.get(reverse('author_delete', kwargs={'pk': 1}))
        self.assertEqual(resp.status_code, 200)
        self.assertTemplateUsed(template_name='catalog/author_confirm_delete.html')

    def test_logged_in_with_permission_create_author(self):
        login = self.client.login(
            username='testuser1', password='12345')
        response = self.client.post(reverse('author_create'),
                                    {'first_name': 'Christian Name', 'second_name': 'Surname'})
        self.assertEqual(302, response.status_code)
        self.assertEqual('/catalog/authors/', response.url)
...
class RenewBookInstanceViewTest(TestCase):
    ...
    def test_form_invalid_renewal_date_future(self):
        ...
        self.assertEqual( resp.status_code,200)
        self.assertFormError(resp.context['form'], 'renewal_date', 'Invalid date - renewal date more than 4 weeks')
